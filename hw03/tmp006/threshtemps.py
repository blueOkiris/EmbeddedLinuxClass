...
def main():
    ...
    i2c.writeto(tempAddr, highSetBytes)
    i2c.writeto(tempAddr, lowSetBytes)
    ...
    while True:
        ...
        rawTemps = bytearray(2)
        thresholds = [ bytearray(2), bytearray(2) ]

        i2c.writeto(tempAddr, reset)
        i2c.readfrom_into(tempAddr, rawTemps)
        
        i2c.writeto(tempAddr, highAddr)
        i2c.readfrom_into(tempAddr, thresholds[0])
        i2c.writeto(tempAddr, lowAddr)
        i2c.readfrom_into(tempAddr, thresholds[1])

        print(
            'Device: Temp = ' \
            + str(rawToCelsius(rawTemps)) + ', Value: ' \
